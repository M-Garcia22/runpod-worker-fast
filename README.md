...
*   **entrypoint.sh**: Performs initialization tasks when the worker starts (downloads ~42 GB of models on first run).
*   **new_Wan22_api.json**: Single workflow file supporting up to 4 LoRA pairs for Wan2.2 image-to-video generation.

### 🏗️ Building the Docker Image

**Important**: The Docker image requires significant disk space during build (~10-15 GB). Models are downloaded at runtime (~42 GB) to keep the image size manageable.

#### Option 1: Build on RunPod (Recommended)

1. Create a RunPod pod with Docker support (any GPU pod with 50+ GB disk space)
...
#### Option 2: Build on Cloud Instance

Use any cloud provider (AWS, GCP, Azure) with Docker installed:
```bash
git clone <your-repo>
cd runpod-worker
docker buildx build --platform linux/amd64 -t mgarcia8324/openmind-wan22:optimized --push .
```

#### Option 3: Build Locally (Requires 50+ GB free space)

```bash
docker buildx build --platform linux/amd64 -t mgarcia8324/openmind-wan22:optimized --push .
```

**Note**: Models download automatically on first container start via `entrypoint.sh`. This means:
- First run: Downloads ~42 GB (takes time depending on connection)
- Subsequent runs: Skips download if models exist
- To persist models between restarts, use Docker volumes
...
**See [RUNPOD_SETUP.md](RUNPOD_SETUP.md) for detailed step-by-step instructions.**

Quick steps:
1. Push this repository to GitHub
2. Go to RunPod Console → Serverless → New Endpoint
3. Connect your GitHub repository
4. Set **Container Disk to 100-150 GB** (models are ~42 GB)
5. **Recommended**: Mount a Network Volume at `/ComfyUI/models` to persist models between cold starts
6. Deploy and wait for build to complete
7. Test your endpoint

**Important**: First request will be slow (~5-10 min) as it downloads ~42 GB of models. Use a Network Volume to avoid re-downloading on each cold start.

Once the endpoint is active, submit jobs via HTTP POST requests according to the API Reference above.
